            ...
            if (!account.isLogin()) {
                ...
                if (FPI.code_verify.get(p.getUniqueId())!=null) {
                    if (!account.changePassword(args[0])) {
                        sender.sendMessage(fpi.putLanguage("PasswordEquals","&c原密码不能与新密码一样！"));
                        return true;
                    }
                    if (!args[0].equals(args[1])) {
                        sender.sendMessage(fpi.putLanguage("PasswordNotEquals","&c密码不一！"));
                        return true;
                    }
                    sender.sendMessage(fpi.putLanguage("AccountChangePassword", "&a密码成功换成： %password%！").replace("%password%", args[1]));
                    return true;
                }
                sender.sendMessage(fpi.putLanguage("NoLoginChangePassword","&c你没有获取验证码！"));
            } else {
                if (args.length < 3) {
                    sender.sendMessage(fpi.subcmd());
                    return true;
